...
async def setup_google_sheet():
    ...
    try:
        sheet = await asyncio.wait_for(asyncio.to_thread(connect), timeout=30)
        return sheet
...
async def scrape_page(session, url, fallback, semaphore):
...
async def main():
    ...
    if sheet:
        try:
            # Column 5 = Link column
            existing_links = set(sheet.col_values(5)[1:])
        except Exception as e:
            log(f"⚠️ Could not load existing links: {e}")
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        ...
        # Save to Sheet (9 columns exactly)
        if sheet and jobs:
            try:
                rows = []
